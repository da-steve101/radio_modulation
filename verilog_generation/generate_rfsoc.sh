...
python3 compute_vgg_with_csv.py --model_name $MODEL_DIR --nu_conv 1.2 --nu_dense 0.7 --prec $PREC \
 --bn_p $BN_PREC --wr_files --no_filts 64,64,64,64,64,64,64,128,128,24 --img_prec $IMG_PREC
...
for i in {1..2}
do
python3 generate_bn_vecs.py --file_in $MODEL_DIR/vgg_bn_dense_$i"_a_b.csv" --file_out $MODEL_DIR/bnd$i.sv --bn_id $i --rshift $BN_PREC --bw_in 16 --bw_out 16
...
rsync -aP $MODEL_DIR/*.sv ../verilog/*.sv --exclude tw_vgg.sv --exclude tw_vgg_2iq_bin.sv --exclude tw_vgg_2iq_incr.sv tuna:~/rt_amc_models/rfsoc/srcs/
